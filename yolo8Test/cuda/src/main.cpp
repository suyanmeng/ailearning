...
Mat preprocess(const Mat& img, int input_w, int input_h, float& scale,
               int& pad_w, int& pad_h) {
    ...
    split(
        padded,
        channels);  // 3通道变1通道，每个通道都是HWC格式的单通道图像，高度和宽度不变，将3个单通道叠加起来，高度就变为了640*3
    Mat chw;
    vconcat(channels[0], channels[1],
            chw);  // RGBRGBRGB->RRRGGGBBB也就实现了HWC -> CHW
...
void postprocess(const float* output, const int input_w, const int input_h,
                 ...
                 vector<Box>& detections, float conf_thresh = 0.25f,
                 float nms_thresh = 0.45f) {
    cout << input_w << ',' << input_h << ',' << scale << ',' << pad_w << ','
         << pad_h << endl;
    // 打印前10个框的坐标和置信度
    cout << "调试：前10个框的输出值：" << endl;
    for (int i = 0; i < 10; i++) {
        cout << "  框" << i << ": cx=" << output[i]
             << ", cy=" << output[8400 + i] << ", conf=" << output[4 * 8400 + i]
             << endl;
    }
    ...
    for (int i = 0; i < num_anchors; i++) {
        // 1. 提取坐标：x_center, y_center, w,
        // h（严格按[1,84,8400]排布）84行，8400列，84个特征值，8400个框，
        // 零点在左上角，向下向右为正，框的标记为左上右下
    ...
    for (int idx : indices) {
        detections.push_back(
            {static_cast<float>(boxes[idx].x), static_cast<float>(boxes[idx].y),
             static_cast<float>(boxes[idx].x + boxes[idx].width),
             static_cast<float>(boxes[idx].y + boxes[idx].height), scores[idx],
             class_ids[idx]});
    }
}
    ...
    for (int c = 0; c < 3; ++c) {
        for (int h = 0; h < dst_h; ++h) {
            for (int w = 0; w < dst_w; ++w) {
                ...
                if (c == 0)
                    output_img.at<cv::Vec3f>(h, w)[2] = val;  // R
                else if (c == 1)
                    output_img.at<cv::Vec3f>(h, w)[1] = val;  // G
                else
                    output_img.at<cv::Vec3f>(h, w)[0] = val;  // B
            }
        }
    }
    cv::Mat output_vis;
    output_img.convertTo(
        output_vis, CV_8UC3,
        255.0);  // 8位无符号型3通道，计算要用浮点，写图片要用整形
